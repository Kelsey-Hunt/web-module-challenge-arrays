...
function removeFlavorByName(removeArray, removedFlavor){
  const removedIndex = removeArray.indexOf(removedFlavor);
  removeArray.splice(removedIndex, 1);
  return removeArray;
}
...
function getAverageWordLength(avgWords){
  let words = 0;
  let totalWords = 0;
  for (let i = 0; i < avgWords.length; i++) {
    let item = avgWords[i];
    const split = item.split(' ');
    const wordCount = split.filter(word => word !== '').length;
    totalWords = totalWords + wordCount;
  }
  return totalWords/(avgWords.length);
}
...
function getRandomFlavors(list1, list2, list3, list4){
  ...
  for(let a = 0; a < list1.length; a++){
    mainList.push(list1[a]);
  }
  
  for(let b = 0; b < list2.length; b++){
    mainList.push(list2[b]);
  }
  
  for(let c = 0; c < list2.length; c++){
    mainList.push(list2[c]);
  }
  
  for(let d = 0; d < list2.length; d++){
    mainList.push(list2[d]);
  }
  
  while(randomList.length < 31){
    let randomNum = (Math.ceil(Math.random() * mainList.length - 1));
    let randomFlavor = mainList[randomNum];
    if(!randomList.includes(randomFlavor)){
      randomList.push(randomFlavor);
    }
  }
  return randomList;
}
